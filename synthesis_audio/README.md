...
* Read the translated text from the AWS S3 bucket `megapipeline-s3bucket` and folder `text_translated`
* Use Eleven Labs API to generate an audio file in French or any other language 
* Save the audio mp3 file in bucket `megapipeline-s3bucket` and folder `output_audios` (use the same file name and change the extension to .mp3)
...
```
docker run --rm -ti -v "$(pwd)":/app --env-file .env synthesis_audio
```
...
* Verify that your uploaded data shows up in the [Mega Pipeline App](https://ai5-mega-pipeline.dlops.io/) # TODO: Add link to the app

### NOT OPTIONAL: You can use the cli_11.py script to synthesize audio with Eleven Labs. Eleven Labs allows you to train voice models.
